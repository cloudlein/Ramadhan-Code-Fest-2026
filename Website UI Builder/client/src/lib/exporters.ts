import type { BuilderProject, UINode } from '@builder/shared';
...
const mapCommonClasses = (node: UINode): string[] => {
  ...
  classes.push(
    node.style.textAlign === 'left' ? 'text-left' : node.style.textAlign === 'center' ? 'text-center' : 'text-right'
  );

  return classes;
};

const inlineStyleFromNode = (node: UINode): string => {
...
const renderInnerText = (node: UINode): string => {
  if (node.type === 'input') {
...
const renderNodeHtml = (project: BuilderProject, nodeId: string): string => {
...
const renderNodeReact = (project: BuilderProject, nodeId: string, depth = 2): string => {
  const node = project.nodesById[nodeId];
  if (!node) {
    return '';
  }
...
export const exportProject = (project: BuilderProject): ExportPayload => {
