...
import type { BuilderProject, ComponentCategory, ComponentType, UINode } from '@builder/shared';
...
const spacing = (top: number, right = top, bottom = top, left = right) => ({ top, right, bottom, left });

const typeCategoryMap: Record<ComponentType, ComponentCategory> = {
  container: 'layout',
  section: 'layout',
  grid: 'layout',
  spacer: 'layout',
...
function createProjectShell(name: string): BuilderProject {
...
function applyTypographyScale(node: UINode): void {
  const nodeName = node.name.toLowerCase();

  const isHeroTitle =
    nodeName.includes('hero heading') ||
...
function estimateWrappedLines(text: string, width: number, fontSize: number): number {
  ...
  return blocks.reduce((total, block) => {
    ...
    if (!content.length) return total + 1;
    return total + Math.max(1, Math.ceil(content.length / charsPerLine));
  }, 0);
}

function ensureTextNodeHeight(node: UINode): void {
...
function polishTemplate(project: BuilderProject, rootBackground: string): BuilderProject {
  const next = JSON.parse(JSON.stringify(project)) as BuilderProject;
  ...
  Object.values(next.nodesById).forEach((node) => {
    if (node.id === next.rootNodeId) return;
...
function buildSaasLanding(): BuilderProject {
...
function buildPortfolioCreator(): BuilderProject {
...
function buildStorefrontShop(): BuilderProject {
...
function buildAuthLogin(): BuilderProject {
...
function buildAuthRegister(): BuilderProject {
...
export function buildProjectFromPreset(presetId: string): BuilderProject {
  if (presetId === 'saas-landing') return buildSaasLanding();
  if (presetId === 'portfolio-creator') return buildPortfolioCreator();
  if (presetId === 'storefront-shop') return buildStorefrontShop();
  if (presetId === 'auth-login') return buildAuthLogin();
  if (presetId === 'auth-register') return buildAuthRegister();
