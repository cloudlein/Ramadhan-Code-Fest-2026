...
import type { ComponentType } from '@builder/shared';
...
interface BuilderState {
  ...
  addNode: (type: ComponentType, position: { x: number; y: number }, parentId?: string | null) => void;
  updateNodeFrame: (id: string, frame: Partial<NodeFrame>, pushHistory?: boolean) => void;
  updateNodeName: (id: string, name: string) => void;
  updateNodeStyle: (id: string, style: Partial<NodeStyle>) => void;
  updateNodeContent: (id: string, content: Partial<UINode['content']>) => void;
  setNodeParent: (id: string, parentId: string | null) => void;
  removeNode: (id: string) => void;
  duplicateNode: (id: string) => void;
  setAlignmentGuides: (guides: AlignmentGuide[]) => void;
  clearAlignmentGuides: () => void;
  updateSettings: (settings: Partial<BuilderProject['settings']>) => void;
...
function createRootProject(name = 'Untitled Project'): BuilderProject {
...
function normalizeProject(project: BuilderProject): BuilderProject {
...
function absolutePosition(project: BuilderProject, nodeId: string): { x: number; y: number } {
  let x = 0;
  let y = 0;
  let cursor: string | null = nodeId;
  while (cursor) {
    const node: UINode | undefined = project.nodesById[cursor];
    if (!node) break;
    x += node.frame.x;
    y += node.frame.y;
...
function appendChild(project: BuilderProject, parentId: string, childId: string): void {
...
function removeChild(project: BuilderProject, parentId: string, childId: string): void {
  const parent = project.nodesById[parentId];
  if (!parent) {
    return;
  }
...
function collectSubtreeIds(project: BuilderProject, rootId: string): string[] {
  const ids: string[] = [];
  const stack = [rootId];
  while (stack.length) {
...
function cloneSubtree(project: BuilderProject, rootId: string): { nodes: Record<string, UINode>; rootNewId: string } {
...
function withHistory(get: () => BuilderState, set: (fn: (state: BuilderState) => Partial<BuilderState>) => void, action: (draft: BuilderProject) => void): void {
...
export const useBuilderStore = create<BuilderState>((set, get) => ({
  ...
  setNodeParent: (id, parentId) => {
    withHistory(get, set, (project) => {
      ...
      if (!containerTypes.has(project.nodesById[nextParent].type)) return;
      if (nextParent === node.id) return;
  ...
  removeNode: (id) => {
    withHistory(get, set, (project) => {
      if (id === project.rootNodeId) return;
      const node = project.nodesById[id];
      if (!node) return;
  ...
  setProject: (project) =>
    set({ project: normalizeProject(project), selectedId: null, history: { past: [], future: [] } }),
...
export function viewportSizeForMode(mode: DeviceMode): number {
  if (mode === 'mobile') return 375;
  if (mode === 'tablet') return 768;
  return 1440;
}

export function suggestInitialFrame(type: ComponentType): NodeFrame {
