...
import type { UINode } from '@builder/shared';

interface NodeViewProps {
  node: UINode;
  absoluteFrame: UINode['frame'];
  selected: boolean;
  previewMode: boolean;
  hideLabel?: boolean;
  onSelect: (event: React.MouseEvent, nodeId: string) => void;
  onPointerDownMove: (event: React.PointerEvent, nodeId: string) => void;
  onPointerDownResize: (
...
function getRibbonColor(seed: string): { background: string; border: string; text: string } {
  const palettes = [
...
function getNodeContent(node: UINode, previewMode: boolean): React.ReactNode {
  if (node.type === 'input') {
    if (previewMode) {
      return (
        <input
          className="h-full w-full bg-transparent outline-none"
          placeholder={node.content.placeholder ?? 'Input'}
          defaultValue=""
        />
      );
    }
    return <div className="opacity-75">{node.content.placeholder ?? 'Input'}</div>;
  }
    ...
    if (previewMode) {
      return (
        <textarea
          ...
          placeholder={node.content.placeholder ?? 'Textarea'}
          defaultValue=""
        />
      );
    }
    return <div className="opacity-75">{node.content.placeholder ?? 'Textarea'}</div>;
  }
  if (node.type === 'select') {
    if (previewMode) {
    ...
    return <div className="opacity-75">{(node.content.options ?? ['Option']).join(' | ')}</div>;
  }
  ...
  if (node.type === 'dropdown' && previewMode) {
    return (
      <button
        type="button"
        className="h-full w-full bg-transparent text-left text-inherit"
        onClick={(event) => event.preventDefault()}
      >
        {node.content.text ?? 'Dropdown'}
      </button>
    );
  }
  if (node.type === 'accordion' && previewMode) {
    return (
      <details className="h-full w-full">
        <summary className="cursor-pointer">{node.content.label ?? 'Accordion'}</summary>
        <div className="pt-2 text-sm">{node.content.text ?? 'Accordion content'}</div>
      </details>
    );
  }
  if (node.type === 'image') {
    return node.content.src ? (
      <img src={node.content.src} alt={node.content.alt ?? 'image'} className="h-full w-full object-cover" draggable={false} />
    ) : (
      <div className="flex h-full w-full items-center justify-center text-xs text-textMuted">Image</div>
    );
  }
  ...
  const text = hasText ? (node.content.text as string) : allowNameFallback.has(node.type) ? node.name : '';
  if (!text) return null;
  return <span className="whitespace-pre-wrap">{text}</span>;
}

function handlePositionClass(direction: string): string {
  switch (direction) {
    case 'n':
...
export function NodeView({
  ...
  return (
    <div
      ...
      style={style}
      className={`group ${selected ? 'ring-1 ring-accent ring-offset-1 ring-offset-[#0b1220]' : ''} ${previewMode ? '' : 'cursor-grab active:cursor-grabbing'}`}
    ...
    >
      {showRibbon ? (
        <span
          className="pointer-events-none absolute left-3 top-0 z-[-1] translate-y-[2px] rounded-b-md border border-t-0 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.08em] opacity-0 transition-[transform,opacity] duration-200 group-hover:translate-y-[-100%] group-hover:opacity-100"
          style={{
      ...
      <div className="relative z-[2] h-full w-full overflow-hidden" style={{ borderRadius: 'inherit' }}>
        {getNodeContent(node, previewMode)}
      </div>
      ...
      {!previewMode && selected
        ? resizeHandles.map((direction) => (
            <span
              ...
              onPointerDown={(event) => onPointerDownResize(event, node.id, direction)}
              className={`absolute z-[3] h-5 w-5 -m-2 flex items-center justify-center rounded-md ${handlePositionClass(direction)}`}
            >
              <span className="h-3.5 w-3.5 rounded-sm border border-white bg-accent" />
            </span>
          ))
        : null}
    </div>
