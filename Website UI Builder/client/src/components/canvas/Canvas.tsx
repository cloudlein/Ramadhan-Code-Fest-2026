...
import type { ComponentType, UINode } from '@builder/shared';
...
export function Canvas() {
  ...
  const [interactionCursor, setInteractionCursor] = useState<string>('default');
  const [isResizing, setIsResizing] = useState(false);

  const visibleNodes = useMemo(
    () => Object.values(project.nodesById).filter((node) => node.id !== project.rootNodeId),
    [project.nodesById, project.rootNodeId]
  );
  ...
  const getCanvasPoint = (event: React.PointerEvent | React.DragEvent): { x: number; y: number } => {
  ...
  const parentAbsolute = (nodeId: string): { x: number; y: number } => {
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (active.kind === 'move') {
      ...
      const dropContainer = findBestContainerAtPoint(
        project,
        { x: snapped.x + nextAbsolute.width / 2, y: snapped.y + Math.min(nextAbsolute.height / 2, 40) },
        absoluteFrames,
        active.nodeId
      );
  ...
  return (
    <main className="flex h-full flex-1 flex-col bg-surface">
      <div className="border-b border-border bg-[#101722] px-4 py-2 text-xs text-textMuted">
        <span className="inline-flex items-center gap-2">
          <span className="flex h-4 w-4 items-center justify-center rounded border border-blue-500/40 bg-blue-500/15 text-[9px] font-bold text-blue-300">
            CV
          </span>
          Canvas mode: {previewMode ? 'Preview' : 'Edit'} | Viewport: {deviceMode} ({viewportWidth}px) | Zoom:{' '}
          {Math.round(zoomLevel * 100)}%
        </span>
      </div>

      <div className="builder-scroll flex-1 overflow-auto p-4">
        <div
          ref={containerRef}
          className="relative mx-auto border border-border bg-[#11151c]"
          style={{
        ...
        >
          <div
            className="builder-grid absolute left-0 top-0 origin-top-left bg-[#11151c]"
            style={{
              width: viewportWidth,
          ...
          >
            {visibleNodes.map((node) => (
              <NodeView
                key={node.id}
                node={node}
                absoluteFrame={absoluteFrames[node.id] ?? node.frame}
                selected={selectedId === node.id}
                previewMode={previewMode}
                hideLabel={isResizing}
            ...
            {alignmentGuides.map((guide, index) => (
              <span
                ...
                className="pointer-events-none absolute z-[80] bg-accent/90"
                style={
            ...
            {dropTargetId && absoluteFrames[dropTargetId] ? (
              <div
